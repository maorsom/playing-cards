...
InputManager::~InputManager() { LogVerbose("Shutting down InputManager"); }
...
void InputManager::OnMouseButtonPressed(int button, int action, int mods) {
  ...
  glm::vec2 worldPos = gRenderer->Deproject(
      glm::vec2(xpos / (float)windowSizeX, 1.0f - (ypos / (float)windowSizeY)));
