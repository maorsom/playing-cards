...
CardStack::CardStack(const std::vector<CardDef> &inDeck,
                     unsigned int startIndex /*= 0*/,
                     unsigned int count /*= 0*/) {
  if (count == 0)
    count = inDeck.size() - startIndex;
...
void CardStack::OnClicked(double mouseX, double mouseY, int button, int action,
                          int mods) {
  if (action != 0)
    return;
...
void CardStack::UpdateClickableArea() {
  int offset = Size() - 1;
  SetClickableArea(Rect(m_Location.x - 14.0f,
